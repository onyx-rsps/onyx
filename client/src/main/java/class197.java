public final class class197 extends Node {
   boolean field2238;
   boolean field2239;
   boolean field2240;
   class192 field2227;
   class194 field2235;
   class197 field2241;
   class206 field2245;
   class210 field2232;
   class212 field2229;
   class214 field2230;
   class215[] field2234;
   int field2223;
   int field2224;
   int field2225;
   int field2226;
   int field2228;
   int field2231;
   int field2233;
   int field2236;
   int field2237;
   int field2243;
   int field2244;
   int[] field2242;

   class197(int var1, int var2, int var3) {
      this.field2234 = new class215[5];
      this.field2242 = new int[5];
      this.field2236 = 0;
      this.field2226 = this.field2237 = var1;
      this.field2224 = var2;
      this.field2225 = var3;
   }

   static void method3412(int var0, class245 var1, boolean var2) {
      class168 var4 = class186.method3221().method5511(var0);
      int var5 = class20.field140.field999;
      int var6 = (class20.field140.field1069 >> 7) + class281.field3625;
      int var7 = (class20.field140.field1059 >> 7) + class78.field1134;
      class245 var8 = new class245(var5, var6, var7);
      class186.method3221().method5499(var4, var8, var1, var2);
   }

   static void method3413(int var0, int var1) {
      class229 var3 = class149.method2495(class227.field2614, client.field453.field1205);
      var3.field2691.method6122(var1);
      var3.field2691.method5972(var0);
      client.field453.method1963(var3);
   }
}
